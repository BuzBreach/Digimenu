...
exports.getRecommendationsForCustomer = getRecommendationsForCustomer;
 ...
 */
async function getRecommendationsForCustomer(mobile) {
    try {
        ...
        const orderCountMap = {};
        const categoryScore = {};
        const queueScore = { KITCHEN: 0, BAR: 0 };
        let vegScore = 0;
        let nonVegScore = 0;
        ...
        customer.orders.forEach((order, orderIndex) => {
            ...
            order.items.forEach((item) => {
                ...
                if (item.menuItem.isVeg)
                    vegScore += item.quantity * recencyBoost;
                else
                    nonVegScore += item.quantity * recencyBoost;
            });
        });
        const scoreMap = new Map();
        const addScore = (item, score, reason) => {
            if (!item.isAvailable)
                return;
        ...
        if (lastOrderItemIds.length > 0) {
            const lastVisitItems = await db_1.prisma.menuItem.findMany({
                where: {
        ...
        if (ruleRecommendedNames.length > 0) {
            const ruleItems = await db_1.prisma.menuItem.findMany({
                where: {
        ...
        const availableItems = await db_1.prisma.menuItem.findMany({
        ...
        availableItems.forEach((item) => {
            ...
            if (prefersVeg && item.isVeg)
                score += 18;
            if (!prefersVeg && !item.isVeg && nonVegScore > 0)
                score += 12;
            if (!orderedItemIds.has(item.id) && categoryAffinity > 0)
                score += 10;
            if (score > 0) {
                addScore(item, score, orderedItemIds.has(item.id) ? 'customer favorite' : `matches ${item.category?.name || 'taste'} preference`);
            }
        });
        ...
        if (orderedItemIds.size > 0) {
            ...
            const relatedOrders = await db_1.prisma.order.findMany({
                where: {
        ...
        if (sortedUserOrderHistory.length > 0) {
            ...
            const favoriteItems = await db_1.prisma.menuItem.findMany({
                where: {
...
async function getBestSellers() {
    try {
        return await db_1.prisma.menuItem.findMany({
            where: {
                isAvailable: true,
                category: {
