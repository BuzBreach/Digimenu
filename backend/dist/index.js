...
const DEFAULT_CAFE_SETTINGS = {
    ...
    tableCount: 20,
    backupFolder: path_1.default.resolve(process.cwd(), 'backups'),
};
...
const emitOrderStatus = (order) => {
    ...
    io.emit(statusEventMap[order.status] || 'order:updated', order);
};
const emitPaymentStatus = (payment, order) => {
    io.emit('payment:updated', payment);
    if (order)
        emitOrderStatus(order);
};
const createRateLimit = (name, limit, windowMs) => (req, res, next) => {
...
const getCafeSettings = async () => {
    const row = await db_1.prisma.appSetting.findUnique({ where: { key: 'cafeProfile' } });
    return { ...DEFAULT_CAFE_SETTINGS, ...(row?.value || {}) };
};
const saveCafeSettings = async (settings) => {
    const cleanSettings = {
    ...
    await db_1.prisma.appSetting.upsert({
...
const buildBackupPayload = async () => {
    const [customers, categories, menuItems, orders, recommendations, users, feedback, inventoryTransactions, settings] = await Promise.all([
        db_1.prisma.customer.findMany({ orderBy: { createdAt: 'asc' } }),
        db_1.prisma.menuCategory.findMany({ orderBy: { sortOrder: 'asc' } }),
        db_1.prisma.menuItem.findMany({ orderBy: { name: 'asc' } }),
        db_1.prisma.order.findMany({ include: { items: true, payments: true }, orderBy: { createdAt: 'asc' } }),
        db_1.prisma.recommendation.findMany({ orderBy: { shownAt: 'asc' } }),
        db_1.prisma.adminUser.findMany({
            select: { id: true, username: true, name: true, role: true, createdAt: true, updatedAt: true },
            orderBy: { createdAt: 'asc' },
        }),
        db_1.prisma.customerFeedback.findMany({ orderBy: { createdAt: 'asc' } }),
        db_1.prisma.inventoryTransaction.findMany({ orderBy: { createdAt: 'asc' } }),
        getCafeSettings(),
    ]);
    return {
        app: 'Niva POS',
...
const cleanText = (value, max = MAX_NOTE_LENGTH) => typeof value === 'string' ? value.replace(/[\u0000-\u001F\u007F]/g, ' ').trim().slice(0, max) : undefined;
...
const generatePseudoQrSvg = (value) => {
    const size = 33;
    const cell = 10;
    const margin = 4;
    let seed = 0;
...
const buildCustomerProfile = async (mobile, name) => {
    ...
    const orderHistory = await db_1.prisma.order.findMany({
    ...
    const recommendations = await (0, recommend_1.getRecommendationsForCustomer)(normalizedMobile);
    if (recommendations.length > 0) {
        const existingRecommendationKeys = new Set((await db_1.prisma.recommendation.findMany({
            where: {
        ...
        const newRecommendationRows = recommendations.map((item, index) => ({
            customerId: customer.id,
            menuItemId: item.id,
            source: orderHistory.length > 0 && index < orderHistory[0].items.length ? prisma_paid_1.RecommendationSource.LAST_ORDER : prisma_paid_1.RecommendationSource.RULE_BASED,
        ...
        if (newRecommendationRows.length > 0) {
            await db_1.prisma.recommendation.createMany({ data: newRecommendationRows, skipDuplicates: true });
        }
    }
    return {
        customer,
...
const getAdminFromRequest = (req) => {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer '))
        return null;
    try {
        return jsonwebtoken_1.default.verify(authHeader.split(' ')[1], JWT_SECRET);
    }
    catch {
        return null;
    }
...
const getDateSeries = (fromDate, toDate) => {
...
const getOrderNumberPrefix = (date) => {
    const year = String(date.getFullYear()).slice(-2);
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const day = String(date.getDate()).padStart(2, '0');
    return `#${year}${month}${day}-`;
};
const isPhonePeConfigured = () => PAYMENT_GATEWAY === 'phonepe' &&
    Boolean(PHONEPE_CLIENT_ID && PHONEPE_CLIENT_SECRET && PHONEPE_MERCHANT_ID);
const phonePeRequest = async (path, options = {}) => {
...
const getPhonePePaymentState = (payload) => {
    const state = String(payload?.state || payload?.data?.state || payload?.code || '').toUpperCase();
    if (['COMPLETED', 'PAYMENT_SUCCESS', 'SUCCESS', 'PAID'].includes(state))
        return 'PAID';
    if (['FAILED', 'PAYMENT_ERROR', 'PAYMENT_DECLINED'].includes(state))
        return 'FAILED';
    if (['CANCELLED', 'PAYMENT_CANCELLED'].includes(state))
        return 'CANCELLED';
    return 'PENDING';
};
const extractPhonePePaymentId = (payload) => payload?.paymentDetails?.[0]?.transactionId ||
    payload?.data?.paymentDetails?.[0]?.transactionId ||
    payload?.transactionId ||
    payload?.data?.transactionId ||
    null;
const markOrderPaid = async (orderId, paymentMethod, gatewayFields = {}) => {
    const paidOrder = await db_1.prisma.order.update({
        where: { id: orderId },
        data: {
            isPaid: true,
            paidAt: new Date(),
...
const verifyPhonePeWebhook = (req) => {
    ...
    if (saltKey && saltIndex && xVerify) {
        ...
        const expectedHash = crypto_1.default.createHash('sha256').update(`${rawBody}${saltKey}`).digest('hex');
...
const validateOrderItems = (rawItems, menuItemMap) => {
...
app.post('/api/orders', orderRateLimit, async (req, res) => {
    const { customerId, tableNumber, notes, items, // [{ menuItemId, quantity, notes, addOnsSelected }]
     } = req.body;
    ...
    try {
        ...
        const customerExists = await db_1.prisma.customer.findUnique({ where: { id: customerId }, select: { id: true } });
        if (!customerExists)
        ...
        const menuItemIds = Array.isArray(items) ? items.map((i) => i.menuItemId).filter(isUuid) : [];
        ...
        const savedOrder = await db_1.prisma.$transaction(async (tx) => {
            ...
            const createdOrder = existingOpenOrder ||
            ...
            await Promise.all(Array.from(stockUsage.entries()).map(async ([menuItemId, quantity]) => {
                const menuItem = menuItemMap.get(menuItemId);
                if (!menuItem?.trackStock)
                    return null;
            ...
            if (existingOpenOrder) {
                const appendedNotes = notes && cleanText(notes) !== existingOpenOrder.notes
        ...
        const populatedOrder = await db_1.prisma.order.findUnique({
...
app.post('/api/orders/:id/feedback', async (req, res) => {
    ...
    try {
        ...
        if (customerId !== order.customerId)
            return res.status(403).json({ error: 'Feedback access denied.' });
...
app.post('/api/payments/phonepe/create', async (req, res) => {
    ...
    try {
        ...
        const hostedCheckoutUrl = phonePeResponse.redirectUrl ||
...
app.get('/api/payments/phonepe/status/:merchantOrderId', async (req, res) => {
    ...
    try {
        ...
        const updatedPayment = await db_1.prisma.payment.update({
            where: { merchantOrderId },
            data: {
                status,
        ...
        if (status === 'PAID' && !payment.order.isPaid) {
            ...
            updatedOrder = await db_1.prisma.order.update({
...
app.post('/api/orders/:id/cancel', async (req, res) => {
    ...
    try {
        ...
        const cancelledOrder = await db_1.prisma.$transaction(async (tx) => {
...
app.post('/api/admin/login', loginRateLimit, async (req, res) => {
    ...
    try {
        const user = await db_1.prisma.adminUser.findUnique({ where: { username } });
        if (!user) {
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id, username: user.username, role: user.role, name: user.name }, JWT_SECRET, { expiresIn: '24h' });
        res.json({
            token,
            user: {
                id: user.id,
...
app.post('/api/admin/change-password', authenticateAdmin, async (req, res) => {
    ...
    try {
        const user = await db_1.prisma.adminUser.findUnique({ where: { id: req.user.id } });
        if (!user)
...
app.get('/api/admin/system/status', authenticateAdmin, async (_req, res) => {
    const settings = await getCafeSettings();
    const provider = (process.env.SMS_PROVIDER || '').toLowerCase();
    const smsConfigured = provider === 'fast2sms'
...
app.get('/api/admin/users', authenticateAdmin, requireRoles('MANAGER'), async (_req, res) => {
    try {
        const users = await db_1.prisma.adminUser.findMany({
            select: { id: true, username: true, name: true, role: true, createdAt: true, updatedAt: true },
            orderBy: { createdAt: 'asc' },
        });
        res.json(users);
...
app.post('/api/admin/users', authenticateAdmin, requireRoles('MANAGER'), async (req, res) => {
    ...
    try {
        const user = await db_1.prisma.adminUser.create({
            data: {
                username,
                name,
                role: normalizedRole,
...
app.delete('/api/admin/users/:id', authenticateAdmin, requireRoles('MANAGER'), async (req, res) => {
...
app.get('/api/admin/staff/attendance', authenticateAdmin, async (req, res) => {
    ...
    try {
        const shifts = await db_1.prisma.staffShift.findMany({
            where: {
        ...
        const activeShifts = await db_1.prisma.staffShift.findMany({
...
app.get('/api/admin/staff/attendance.csv', authenticateAdmin, requireRoles('MANAGER'), async (req, res) => {
    const { start, end, dates } = getDateSeries(typeof req.query.from === 'string' ? req.query.from : undefined, typeof req.query.to === 'string' ? req.query.to : undefined);
    try {
        const [users, shifts] = await Promise.all([
            ...
            db_1.prisma.staffShift.findMany({
                where: { checkInAt: { gte: start, lt: end } },
                orderBy: { checkInAt: 'asc' },
            }),
        ]);
        const shiftsByUserAndDate = new Map();
...
app.post('/api/admin/staff/check-in', authenticateAdmin, async (req, res) => {
    ...
    try {
        const staff = await db_1.prisma.adminUser.findUnique({ where: { id: requestedUserId } });
        if (!staff)
            return res.status(404).json({ error: 'Staff account not found.' });
        const activeShift = await db_1.prisma.staffShift.findFirst({
            where: { userId: requestedUserId, checkOutAt: null },
        });
        if (activeShift)
            return res.status(409).json({ error: `${staff.name} is already checked in.` });
        const shift = await db_1.prisma.staffShift.create({
...
app.post('/api/admin/staff/check-out', authenticateAdmin, async (req, res) => {
    const requestedUserId = req.body.userId || req.user.id;
    if (requestedUserId !== req.user.id && req.user.role !== 'ADMIN' && req.user.role !== 'MANAGER') {
    ...
    try {
        ...
        const shift = await db_1.prisma.staffShift.update({
...
app.put('/api/admin/orders/:id/status', authenticateAdmin, requireRoles('MANAGER', 'KITCHEN', 'BAR', 'BILLING'), async (req, res) => {
    ...
    if (!Object.values(prisma_paid_1.OrderStatus).includes(status)) {
    ...
    try {
        ...
        const refreshedOrder = await db_1.prisma.order.findUnique({
            where: { id },
            include: {
                customer: true,
                items: {
...
app.put('/api/admin/orders/:orderId/items/:itemId/status', authenticateAdmin, requireRoles('MANAGER', 'KITCHEN', 'BAR'), async (req, res) => {
    ...
    try {
        ...
        const allReady = allItems.every((item) => item.status === 'READY' || item.status === 'SERVED' || item.status === 'CANCELLED');
        ...
        await db_1.prisma.kitchenQueue.updateMany({
            where: { orderId },
            data: { status: overallStatus },
        });
        await db_1.prisma.barQueue.updateMany({
            where: { orderId },
            data: { status: overallStatus },
        });
...
app.get('/api/admin/analytics', authenticateAdmin, async (req, res) => {
    try {
        const [allOrders, activeTables, orderItems, allCustomers, customerOrders, recommendationRows] = await Promise.all([
            ...
            db_1.prisma.recommendation.groupBy({
                by: ['source'],
                _count: { source: true },
            }),
        ]);
...
app.get('/api/admin/tables/occupancy', authenticateAdmin, async (_req, res) => {
    try {
        ...
        for (const order of activeOrders) {
            if (!latestByTable.has(order.tableNumber))
...
app.post('/api/admin/backup/save-local', authenticateAdmin, requireRoles('MANAGER'), async (_req, res) => {
    try {
        const settings = await getCafeSettings();
        const backupFolder = path_1.default.resolve(String(settings.backupFolder || DEFAULT_CAFE_SETTINGS.backupFolder));
        await promises_1.default.mkdir(backupFolder, { recursive: true });
        const backup = await buildBackupPayload();
        const filename = `niva-pos-backup-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
        const fullPath = path_1.default.join(backupFolder, filename);
        await promises_1.default.writeFile(fullPath, JSON.stringify(backup, null, 2), 'utf8');
...
app.get('/api/admin/inventory/transactions', authenticateAdmin, requireRoles('MANAGER'), async (_req, res) => {
    try {
        const rows = await db_1.prisma.inventoryTransaction.findMany({
            include: {
...
app.post('/api/admin/inventory/transactions', authenticateAdmin, requireRoles('MANAGER'), async (req, res) => {
...
app.delete('/api/admin/categories/:id', authenticateAdmin, requireRoles('MANAGER'), async (req, res) => {
    const { id } = req.params;
    try {
        await db_1.prisma.menuCategory.delete({ where: { id } });
...
app.post('/api/admin/menu-items', authenticateAdmin, requireRoles('MANAGER'), async (req, res) => {
    const { name, description, price, isVeg, isAvailable, categoryId, targetQueue, addOns, imageUrl, trackStock, stockQuantity, lowStockAt, } = req.body;
